defmodule Jido.Cli.Release.CrossRepoTest do
  use ExUnit.Case, async: true

  alias Jido.Cli.Release.CrossRepo

  setup do
    root = Path.join(System.tmp_dir!(), "jido-cross-repo-test-#{System.unique_integer([:positive])}")
    project = Path.join(root, "jido_cli")
  ...
  test "tests the clean sibling and the exact locked commit once each", context do
    ...
    assert {"JIDO_CLI_JIDOKA_PATH", context.sibling} in elem(sibling_test, 2)[:env]
    assert {"JIDO_CLI_JIDOKA_PATH", nil} in elem(pinned_test, 2)[:env]
  ...
  test "reads one immutable GitHub Jidoka pin from the lock" do
    assert CrossRepo.pinned_ref!() == "f19ce72e7591b3215e832e6e034e3752e84a3604"
  end
end
