defmodule Jido.Console.AutomationTest do
  ...
  test "the signal handler forwards termination through the interrupt boundary" do
    assert {:ok, owner} = Jido.Console.Automation.Interrupt.Signal.init(self())
    assert {:ok, ^owner} = Jido.Console.Automation.Interrupt.Signal.handle_event(:sigterm, owner)
    assert_receive {:jido_console_automation_cancel, :sigterm}
  end
