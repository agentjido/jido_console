defmodule Jido.Console.Session.Reducer do
  ...
  alias Jido.Console.Session.{Envelope, Event, State}
  ...
  defp require_session(%{session_id: session_id}, %Envelope{session_id: session_id}), do: :ok
  ...
  defp put_semantic(state, %Envelope{type: "run_started"} = event) do
    %{state | active_run: event["payload"]}
  end

  defp put_semantic(state, %Envelope{type: type}) when type in ["run_completed", "run_failed"],
    do: %{state | active_run: nil}

  defp put_semantic(state, %Envelope{type: "queue_changed"} = event) do
    queue = if event["payload"]["queue"] == "steering", do: :steering, else: :follow_up
    items = event["payload"]["items"] || []
    %{state | queues: Map.put(state.queues, queue, items)}
  end

  defp put_semantic(state, %Envelope{type: "input_admitted", payload: %{"queue" => queue} = payload}) do
    queue = if queue == "steering", do: :steering, else: :follow_up
    %{state | queues: Map.put(state.queues, queue, payload["items"] || [])}
  end

  defp put_semantic(state, %Envelope{type: "permission_requested", payload: payload}) do
  ...
  defp put_semantic(state, %Envelope{type: "permission_decided", payload: payload}) do
    id = payload["approval_id"]
  ...
  defp put_semantic(state, %Envelope{type: "control_requested", payload: payload}) do
    control = Map.put(payload, "status", "requested")
    %{state | control_state: Map.put(state.control_state, payload["control_id"], control)}
  end

  defp put_semantic(state, %Envelope{type: "control_completed", payload: payload}) do
